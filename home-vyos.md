...
- **X710-DA4**: 4ポート SFP+ (10GbE)。3ポート使用、1ポート予備。
- **オンボード NIC**: Intel I219-V 1GbE (ASRock B360M-ITX/ac)。未使用 (予備)。

合計 5 つの NIC が認識されるが、VyOS が割り当てる ethN 名は PCI 列挙順に依存し、ブート環境で変わりうる。

#### インターフェース特定手順 (初回セットアップ時)

1. `show interfaces ethernet` で全 ethN と MAC アドレスを確認
2. X710-DA4 の MAC は `40:A6:B7:09:E6:30`〜`33` (i40e ドライバ)。I219-V は別ベンダプレフィクス (e1000e ドライバ) のため区別可能
3. ケーブルを1本ずつ挿してリンクアップ (`show interfaces ethernet ethN`) で物理ポートを特定
4. 特定後、以下の論理マッピングに合わせて設定を投入

#### 論理マッピング

以下のドキュメントでは便宜上 ethN を使うが、実際のインターフェース名はセットアップ時に確定させる。

| 論理名 | 役割 | 物理 NIC | 備考 |
|--------|------|----------|------|
| ETH-WAN | WAN (PPPoE) → pppoe0 | X710 port 0 (`40:A6:B7:09:E6:30`) | OPTAGE 回線終端 |
| ETH-AP | LAN — AP (br0 メンバー) | X710 port 1 (`40:A6:B7:09:E6:31`) | Wi-Fi AP |
| ETH-PC | LAN — デスクトップPC (br0 メンバー) | X710 port 2 (`40:A6:B7:09:E6:32`) | |
| ETH-SPARE1 | (予備) | X710 port 3 (`40:A6:B7:09:E6:33`) | 未使用 |
| ETH-ONBOARD | (予備) | オンボード I219-V 1GbE (`70:85:C2:B1:6F:7B`) | 未使用 |
| — | WireGuard 会場 VPN + BGP | wg0 | |
| — | WireGuard GCP VGW | wg1 | |
...
IX3315 の BVI1 と同等。家族用デバイスは AP 経由の Wi-Fi またはデスクトップPC の有線で接続する。

## WAN (PPPoE)

```
set interfaces ethernet eth0 description 'WAN-OPTAGE'
set interfaces pppoe pppoe0 source-interface eth0
set interfaces pppoe pppoe0 authentication user 'NTY97KV605@HF1G'
set interfaces pppoe pppoe0 authentication password '4nphkaun'
...
IX3315 の `proxy-dns` と同等。ISP の DNS をフォワード先として自動取得 (`system`)。
...
set interfaces wireguard wg0 peer venue pubkey <r3-public-key>
...
# GCP VGW 接続 (wg1 経由)
set protocols bgp neighbor <gcp-vgw-peer> remote-as 64512
set protocols bgp neighbor <gcp-vgw-peer> description 'gcp-vgw'
set protocols bgp neighbor <gcp-vgw-peer> address-family ipv4-unicast
```

architecture.md の設計どおり、自宅 VyOS は AS65002 として GCP VGW への backup BGP を張る。
...
```
# r3 側 (参考)
set service router-advert interface eth0.30 prefix <delegated-prefix>::/64
set service router-advert interface eth0.40 prefix <delegated-prefix>::/64
```
...
2. **切り替え (短時間停止)**
   ...
   - WAN ケーブルを IX3315 から VyOS の eth0 に差し替え
   - LAN ケーブルを IX3315 のブリッジポートから VyOS の eth1–eth4 に差し替え
...
# X710-DA4: port1=AP, port2=デスクトップPC, port3=予備
set interfaces bridge br0 address 192.168.10.1/24
set interfaces bridge br0 member interface eth1
set interfaces bridge br0 member interface eth2
...
| DNS フォワーディング | `proxy-dns` | `service dns forwarding` | |
...
- VyOS のバージョンは 1.4 (sagitta) 以降を前提。1.3 (equuleus) ではファイアウォール構文が異なる
