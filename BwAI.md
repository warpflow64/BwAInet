...
flowchart TB
 ...
 subgraph s2["会場"]
        ...
        n5["通信ログ保存 (法執行機関対応)<br>NetFlow v9: r3 → nfcapd (5-tuple, 180日)<br>DNS クエリログ: r3 dns forwarding (qname+client IP)<br>DHCP forensic log: r3 dhcp-server (IP↔MAC↔hostname)<br>NDP dump: r3 cron 1分 (IPv6↔MAC, iOS/Android対応)"]
        r3["Router<br>AS65001<br>DNS forwarding / DHCP / DHCPv6<br>flow-accounting / NDP dump"]
        sw["PoE SW<br>"]
