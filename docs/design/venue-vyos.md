...
| eth1 | vmbr_trunk | Realtek RTL8111H | VLAN トランク (→ PoE スイッチ) |

### VLAN サブインターフェース (eth1)

| VIF | VLAN ID | アドレス (v4) | IPv6 | 用途 |
|-----|---------|--------------|------|------|
| eth1.11 | 11 | 192.168.11.1/24 | なし | 管理 (mgmt) |
| eth1.30 | 30 | 192.168.30.1/24 | DHCPv6-PD /64 | 運営 (staff + live) |
| eth1.40 | 40 | 192.168.40.1/22 | DHCPv6-PD /64 | 来場者 (user) |
...
# VLAN トランク (PoE スイッチ)
set interfaces ethernet eth1 description 'VLAN trunk to PoE switch'

# VLAN 11 (mgmt - v4 only)
set interfaces ethernet eth1 vif 11 address 192.168.11.1/24
set interfaces ethernet eth1 vif 11 description 'VLAN 11 - mgmt'

# VLAN 30 (staff + live)
set interfaces ethernet eth1 vif 30 address 192.168.30.1/24
set interfaces ethernet eth1 vif 30 description 'VLAN 30 - staff + live'

# VLAN 40 (user)
set interfaces ethernet eth1 vif 40 address 192.168.40.1/22
set interfaces ethernet eth1 vif 40 description 'VLAN 40 - user'
...
# VLAN 30
set service router-advert interface eth1.30 prefix <delegated-prefix>::/64 autonomous-flag true
set service router-advert interface eth1.30 managed-flag true
set service router-advert interface eth1.30 other-config-flag true
set service router-advert interface eth1.30 name-server <prefix>::1

# VLAN 40
set service router-advert interface eth1.40 prefix <delegated-prefix>::/64 autonomous-flag true
set service router-advert interface eth1.40 managed-flag true
set service router-advert interface eth1.40 other-config-flag true
set service router-advert interface eth1.40 name-server <prefix>::1
```
...
set firewall ipv4 forward filter rule 10 inbound-interface name eth1.40
...
set firewall ipv4 input filter rule 10 inbound-interface name eth1.40
```
...
set system flow-accounting interface eth1.30
